...
def depthFirstSearch(problem):
    ...
    closed = set()
    fringe = []
    fringe.append([(problem.getStartState(), 'Start', 0)])
    while len(fringe) != 0:
        pop = fringe.pop()
        position = pop[len(pop) - 1][0]
    ...
    while not fringe.isEmpty():
        ...
        if not (position in closed):
            ...
            for i in successors:
                ...
                new_fringe.append(i)
                cost = 0
                for state in new_fringe:
                    cost += state[2]
                fringe.push(new_fringe, cost)
    return []
