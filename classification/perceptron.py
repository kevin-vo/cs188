...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                "*** YOUR CODE HERE ***"
                labelCompare = self.classify(input_train_data[i])
                label = label_train_data[i]
                # x = self.classify(input_train_data)
                # print x
                if label == labelCompare:
                	continue
                self.weights[label] += input_train_data[i]
                self.weights[labelCompare] -= input_train_data[i]
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        lst = self.weights[label].sortedKeys()
        lenLst = len(lst)
        for i in range(0, 100):
        	if i >= lenLst:
        		break
        	best100Features.append(lst[i])
