...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for var in param_vars:
            tup = (var, var - self.learning_rate * grad_tensors[counter])
            counter += 1
            updates.append(tup)
        return updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        counter = 0
        for var in param_vars:
            updateVal = self.momentum * vel_vars[counter] - self.learning_rate * grad_tensors[counter]
            tup = (vel_vars[counter], updateVal)
            tup2 = (var, var + updateVal)
            counter += 1
            updates.append(tup)
            updates.append(tup2)
        return updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        trainData = {model.input_ph: input_train_data, target_ph: target_train_data}
        valData = {model.input_ph: input_val_data, target_ph: target_val_data}
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            # train_loss should be the loss of this iteration using all of the training data
            train_loss = session.run(loss_tensor, trainData)
            session.run(update_ops, trainData)
            # val_loss should be the loss of this iteration using all of the validation data
            val_loss = session.run(loss_tensor, valData)
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        iterateTrain = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        iterateVal = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            inputTrain, targetTrain = iterateTrain.next()
            # training data losses
            train_loss = session.run(loss_tensor, {model.input_ph: inputTrain, target_ph: targetTrain})
            session.run(update_ops, {model.input_ph: inputTrain, target_ph: targetTrain})
            ...
            inputVal, targetVal = iterateVal.next()
            # validation data losses
            val_loss = session.run(loss_tensor, {model.input_ph: inputVal, target_ph: targetVal})
            train_losses.append(train_loss)
            val_losses.append(val_loss)
            if callback is not None: callback(model)
            self.display_progress(iter_, train_losses, val_losses)
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        iterateTrain = MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        iterateVal = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            inputTrain, targetTrain = iterateTrain.next()
